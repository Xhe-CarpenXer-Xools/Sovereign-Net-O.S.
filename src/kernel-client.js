...
(function (global) {
  ...
  class KernelClient extends EventEmitter {
    ...
    async replay(toClockT) {
      if (!this._bridge) return null;
      return this._bridge.replay({ toClockT });
    }

    async at(clockT) {
      if (!this._bridge) return null;
      return this._bridge.at(clockT);
    }

    async verify(stateKey, expectedValue, atClock) {
      if (!this._bridge) return null;
      return this._bridge.verify(stateKey, expectedValue, atClock);
    }
    ...
    async syncPull(peerId, sinceClock = 0) {
      if (!this._bridge) return null;
      return this._bridge.syncPull(peerId, sinceClock);
    }

    /** Deliver a raw pubsub envelope to the sync engine */
    async syncReceive(encoded) {
      if (!this._bridge) return null;
      return this._bridge.syncReceive(encoded);
    }

    async syncStatus() {
      if (!this._bridge) return null;
      return this._bridge.syncStatus();
    }
